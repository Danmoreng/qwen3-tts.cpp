...
static struct ggml_tensor * build_code_pred_decode_block(
    tts_transformer_private * impl,
    struct ggml_context * ctx0,
    struct ggml_cgraph * gf,
    struct ggml_tensor * cur,
    struct ggml_tensor * inp_pos,
    struct ggml_tensor * inp_mrope_pos,
...
struct ggml_cgraph * transformer_internal::ops::build_code_pred_greedy_graph_impl(TTSTransformer & self,
                                                                                  struct ggml_context ** graph_ctx_out) {
    auto & impl = self.impl_;
    const auto & cfg = impl->model.config;
    const int hidden_size = cfg.code_pred_hidden_size;
    ...
    struct ggml_init_params params = {
        /*.mem_size   =*/ impl->state.code_pred_greedy_compute_meta.size(),
        /*.mem_buffer =*/ impl->state.code_pred_greedy_compute_meta.data(),
        /*.no_alloc   =*/ true,
    };
    ...
    struct ggml_tensor * prefill_inp_pos = ggml_new_tensor_1d(ctx0, GGML_TYPE_I32, 2);
    ggml_set_name(prefill_inp_pos, "prefill_inp_pos");
    ggml_set_input(prefill_inp_pos);

    struct ggml_tensor * prefill_inp_mrope_pos = nullptr;
    if (cfg.use_mrope) {
        prefill_inp_mrope_pos = ggml_new_tensor_1d(ctx0, GGML_TYPE_I32, 8);
        ggml_set_name(prefill_inp_mrope_pos, "prefill_inp_mrope_pos");
        ggml_set_input(prefill_inp_mrope_pos);
    }

    struct ggml_tensor * cb0_embd = ggml_get_rows(ctx0, impl->model.codec_embd, inp_cb0_code);
    struct ggml_tensor * hidden_2d = ggml_reshape_2d(ctx0, inp_hidden, in_hidden_size, 1);
    struct ggml_tensor * cur = ggml_concat(ctx0, hidden_2d, cb0_embd, 1);
    ...
    cur = build_code_pred_decode_block(impl.get(), ctx0, gf, cur, prefill_inp_pos, prefill_inp_mrope_pos, nullptr, 2, 0);

    for (int step = 0; step < 15; ++step) {
        struct ggml_tensor * last_hidden = ggml_view_2d(ctx0, cur, hidden_size, 1,
                                                        cur->nb[1], (size_t) (cur->ne[1] - 1) * cur->nb[1]);
        struct ggml_tensor * logits = ggml_mul_mat(ctx0, impl->model.code_pred_head[step], last_hidden);
        struct ggml_tensor * token = ggml_argmax(ctx0, logits);
        ggml_format_name(token, "token_cb%d", step + 1);
        ...
        struct ggml_tensor * step_inp_pos = ggml_new_tensor_1d(ctx0, GGML_TYPE_I32, 1);
        ggml_set_name(step_inp_pos, name);
        ggml_set_input(step_inp_pos);

        struct ggml_tensor * step_inp_mrope_pos = nullptr;
        if (cfg.use_mrope) {
            snprintf(name, sizeof(name), "step%02d_inp_mrope_pos", step + 1);
            step_inp_mrope_pos = ggml_new_tensor_1d(ctx0, GGML_TYPE_I32, 4);
            ggml_set_name(step_inp_mrope_pos, name);
            ggml_set_input(step_inp_mrope_pos);
        }

        snprintf(name, sizeof(name), "step%02d_inp_mask", step + 1);
        struct ggml_tensor * step_inp_mask = ggml_new_tensor_2d(ctx0, GGML_TYPE_F16, impl->state.code_pred_cache.n_ctx, 1);
        ggml_set_name(step_inp_mask, name);
        ggml_set_input(step_inp_mask);

        cur = ggml_get_rows(ctx0, impl->model.code_pred_embd[step], token);
        cur = ggml_reshape_2d(ctx0, cur, in_hidden_size, 1);
        if (impl->model.code_pred_small_to_mtp_weight) {
            cur = ggml_mul_mat(ctx0, impl->model.code_pred_small_to_mtp_weight, cur);
            if (impl->model.code_pred_small_to_mtp_bias) {
        ...
        cur = build_code_pred_decode_block(impl.get(), ctx0, gf, cur, step_inp_pos, step_inp_mrope_pos, step_inp_mask, 1, step + 1);
    }
