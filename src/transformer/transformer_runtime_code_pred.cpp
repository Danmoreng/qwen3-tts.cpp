...
bool TTSTransformer::predict_codes_autoregressive(const float * hidden, int32_t codebook_0_token,
    ...
    const bool greedy_only = temperature <= 0.0f && !trace_frame_enabled;
    if (greedy_only) {
        if (!transformer_internal::ops::ensure_cached_code_pred_greedy_graph(*this)) {
            return false;
        }
        ...
        t0 = clk::now();
#endif
        auto & graph = impl_->state.code_pred_greedy_graph;
        struct ggml_cgraph * gf = graph.graph;
        if (!ggml_backend_sched_alloc_graph(impl_->state.sched, gf)) {
            error_msg_ = "Failed to allocate greedy code predictor graph";
            return false;
        }
#ifdef QWEN3_TTS_TIMING
        t1 = clk::now();
        ...
        t0 = clk::now();
#endif
        ggml_backend_tensor_set(graph.inp_hidden, hidden, 0, cfg.hidden_size * sizeof(float));
        ggml_backend_tensor_set(graph.inp_cb0_code, &codebook_0_token, 0, sizeof(int32_t));

        const int32_t prefill_pos[2] = {0, 1};
        if (graph.prefill_inp_pos) {
            ggml_backend_tensor_set(graph.prefill_inp_pos, prefill_pos, 0, sizeof(prefill_pos));
        }
        if (graph.prefill_inp_mrope_pos && impl_->model.config.use_mrope) {
            const int32_t prefill_mrope_pos[8] = {0, 1, 0, 1, 0, 1, 0, 0};
            ggml_backend_tensor_set(graph.prefill_inp_mrope_pos, prefill_mrope_pos, 0, sizeof(prefill_mrope_pos));
        }

        const ggml_fp16_t zero_fp16 = ggml_fp32_to_fp16(0.0f);
        const ggml_fp16_t neg_inf_fp16 = ggml_fp32_to_fp16(-INFINITY);
        std::vector<ggml_fp16_t> mask((size_t) impl_->state.code_pred_cache.n_ctx, neg_inf_fp16);
        for (int i = 0; i <= 2 && i < impl_->state.code_pred_cache.n_ctx; ++i) {
            mask[(size_t) i] = zero_fp16;
        }

        for (int step = 0; step < 14; ++step) {
            const int32_t n_past = step + 2;
            if (graph.step_inp_pos[(size_t) step]) {
                ggml_backend_tensor_set(graph.step_inp_pos[(size_t) step], &n_past, 0, sizeof(int32_t));
            }
            if (graph.step_inp_mrope_pos[(size_t) step] && impl_->model.config.use_mrope) {
                const int32_t mrope_pos[4] = {n_past, n_past, n_past, 0};
                ggml_backend_tensor_set(graph.step_inp_mrope_pos[(size_t) step], mrope_pos, 0, sizeof(mrope_pos));
            }
            if (n_past < impl_->state.code_pred_cache.n_ctx) {
                mask[(size_t) n_past] = zero_fp16;
            }
            ggml_backend_tensor_set(graph.step_inp_mask[(size_t) step], mask.data(), 0,
...
#endif
        if (ggml_backend_sched_graph_compute(impl_->state.sched, gf) != GGML_STATUS_SUCCESS) {
            error_msg_ = "Failed to compute greedy code predictor graph";
            ggml_backend_sched_reset(impl_->state.sched);
            return false;
        }
#ifdef QWEN3_TTS_TIMING
        ...
        for (int cb = 0; cb < 15; ++cb) {
            ggml_backend_tensor_get(graph.output_tokens[(size_t) cb], &output[(size_t) cb], 0, sizeof(int32_t));
        }
        ggml_backend_sched_reset(impl_->state.sched);
#ifdef QWEN3_TTS_TIMING
        t1 = clk::now();
